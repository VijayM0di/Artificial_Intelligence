...
class PPEKit:
    ...
    def capture_frames(self):
        ...
        logger.debug(f"In Capture frame, RTSP = {self.RTSP_URL}, subscribe = {self.subscribed_topic}, PORT = {PUB_PORT} ")
        ...
        while not self.stop_event.is_set():
            ...
            #except Exception as e:
            #    logger.debug(f"Error in process frames = {e}")
            ...
            logger.debug(f"Inserting Frame")
    ...
    def setup(self):
        ...
        temp_model_yolo = YOLO("models/ppe_final.pt")
        ...
        logger.debug(f'RTSP to PORT = {RTSP_TO_PORT}')
    ...
    def detect(self):
        ...
        model_yolo = YOLO("models/ppe_final.pt")
                ...
                for r in results:
                    ...
                    for box in boxes:
                        ...
                        if conf > 0.5:
                            ...
                            try:
                                xywhs = torch.Tensor([xywh_obj])
                                confss = torch.Tensor([conf])
                                ...
                                if len(outputs) > 0:
                                    ...
                                    rotated_object_points = cv2.transform(
                                        np.array(transformed_downoids).reshape(
                                            -1, 1, 2
                                        ),
                                    ...
                                    for index, downoid in enumerate(
                                        ...
                                        # New Changes
                                        x1, y1, x2, y2 = [
                                            int(i) for i in bbox_xyxy[index]
                                        ]
                                        ...
                                        if not (
                                            ...
                                            if current_object_point not in obj_points:
                                                ...
                                                try:
                                                    current_obj["class"] = currentClass
                                                    current_obj["state"] = currentClass
                                ...
                                logger.debug(f"Exception in detect loop = {e}")
