type_of_violation = 'traffic_rules_illegal_parking'
...
# speed_line_queue = {}
#
# type_of_violation = 'traffic_rules'
# if len(sys.argv) < 2:
...
#
#
# def draw_boxes(img, bbox, names, object_id, identities=None, index=None, offset=(0, 0),
#                downoid=None, bird_view_image=None):
#     global violation
#     global start_time
...
#         if 0 <= abs(avg_speed) <= 5:
#             violation = True
#             if not start_time:
#                 start_time = time.time()
#             x, y = downoid
#             cv2.circle(bird_view_image, (int(x), int(y)), BIG_CIRCLE, COLOR_RED, 2)
#             cv2.circle(bird_view_image, (int(x), int(y)), SMALL_CIRCLE, COLOR_RED, -1)
