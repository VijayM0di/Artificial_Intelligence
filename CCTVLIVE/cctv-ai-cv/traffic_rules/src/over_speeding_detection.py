...
# from new_helpers.predict import (
#    compute_color_for_labels,
#    estimatespeed,
#    UI_box,
#    xyxy_to_xywh
...
# from new_helpers.bird_view_transfo_functions import (
#    xyxy_to_xywh,
#    get_centroids_and_groundpoints,
#    compute_perspective_transform,
#    compute_point_perspective_transformation,
...
# COLOR_RED = get_COLOR_RED()
# COLOR_BLUE = get_COLOR_GREEN()
# COLOR_GREEN = get_COLOR_BLUE()

# BIG_CIRCLE = get_BIG_CIRCLE()
# SMALL_CIRCLE = get_SMALL_CIRCLE()
...
class PPEKit:
    def __init__(self, use_case, frame_queue, stop_event, video_saving_queue):
        ...
        self.speed_limit = UC_TRAFFIC_RULE_SPEED_LIMIT
    ...
    def draw_boxes(
        ...
        try:
            ...
            try:
                ...
                # color = COLOR_BLUE
                color = COLOR_GREEN
                if avg_speed > int(self.speed_limit):
                #if avg_speed > int(10):
                    self.violation = True
                ...
                logger.debug(f"Error calculating speed: {e}")
    ...
    def setup(self):
        ...
        over_speed_limit_response = API(
           RequestMethod.GET,
           OVERSPEED_LIMIT_URL,
           headers={"Authorization": f"Bearer {access_token}"},
        )

        if over_speed_limit_response.status_code == 200:
            overspeed_limit_data = over_speed_limit_response.json()
            self.speed_limit = int(overspeed_limit_data.get("value", UC_TRAFFIC_RULE_SPEED_LIMIT))
            logger.debug(f"Speed Limit received {self.speed_limit}!!")
        else:
            logger.debug(f"Error fetching Speed Limit!!")
        ...
        self.corner_points = np.array(self.corner_points, dtype=np.float64)
    ...
    def detect(self):
        ...
        while self.to_be_checked:
            ...
            if not self.frame_queue.empty():
                ...
                text_1 = "Speed Limit : " + str(UC_TRAFFIC_RULE_SPEED_LIMIT)
                ...
                try:
                    ...
                    if len(outputs) > 0:
                        ...
                        logger.debug("After rotated object points")
                ...
                # time.sleep(0.5)
                logger.debug("Detection process completed")
                avg_time.append(time.time() - start_time)
                logger.debug(f"Took Individual =  {time.time() - start_time}")
                logger.debug(f"Took {sum(avg_time) / len(avg_time)}")
...
if __name__ == "__main__":
    type_of_violation = "traffic_rules_over_speeding"
